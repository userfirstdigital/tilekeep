...
if(!process.argv.includes('--inside')) {
    ...
    for(const name of ['DBUS_SESSION_BUS_ADDRESS','DISPLAY','XAUTHORITY','SESSION_MANAGER','WAYLAND_SOCKET','JOURNAL_STREAM'])delete env[name];
    const runnerLog=fs.openSync(path.join(dir,'runner.log'),'wx',0o600);
    const child=spawn('dbus-run-session',['--',process.execPath,__filename,'--inside'],{env,stdio:['ignore','inherit',runnerLog]});
    ...
    const kwin=spawn('kwin_wayland',['--virtual','--width','1200','--height','800','--no-lockscreen','--no-kactivities','--socket','tilekeep-isolated'],{env,stdio:['ignore',fd,fd]});
